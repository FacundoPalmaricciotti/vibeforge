...
public class AlbumController {
    ...
    public org.springframework.http.ResponseEntity<?> toggleActivoAlbum(@PathVariable Integer id) {
        com.pp2.vibeforge.models.Album album = albumRepository.findById(id).orElse(null);
        
        if (album == null) {
            return org.springframework.http.ResponseEntity.notFound().build();
        }
        
        Boolean estadoActual = album.getActivo() != null ? album.getActivo() : true;
        album.setActivo(!estadoActual);
        
        albumRepository.save(album);
        
        return org.springframework.http.ResponseEntity.ok(album.getActivo());
    }

    @DeleteMapping("/{id}")
    public org.springframework.http.ResponseEntity<?> borrarAlbumDefinitivo(@PathVariable Integer id) {
        com.pp2.vibeforge.models.Album album = albumRepository.findById(id).orElse(null);
