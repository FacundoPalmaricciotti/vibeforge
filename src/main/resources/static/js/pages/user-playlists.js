async function renderUserPlaylists() {
    ...
    for(let i=0; i<8; i++) {
        skPlaylistsHTML += `
            <div class="card" style="background: #181818; padding: 15px; border-radius: 8px; cursor: default; pointer-events: none; display: flex; flex-direction: column;">
                ...
                <div class="skeleton-shimmer sk-rect" style="width: 50%; height: 13px;"></div>
            </div>
        `;
    }
    ...
    const contenidoHTML = `
        <div style="padding: 20px;">
            ...
            <div id="grid-mis-playlists" class="grid">${skPlaylistsHTML}</div>
        </div>
...
async function cargarGrillaPlaylists() {
    ...
    try {
        ...
        for (const pl of misPlaylists) {
            ...
            html += `
                <div class="card animate__animated animate__fadeIn" onclick="navigate('/user/playlist?id=${pl.idPlaylist}')" style="background: #181818;">
        ...
        if (grid) grid.innerHTML = '<p style="color: var(--error); grid-column: 1 / -1;">Error de conexión. No pudimos traer tus listas.</p>';
    }
}
